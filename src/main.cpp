...
int main() {
  ...
  h.onMessage([&lane_no,&ref_vel,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			// Iterating through sensor fusion data and set the ref_vel to lower if we are going to hit the car
			for(int i =0;i<sensor_fusion.size();i++){
				...
				if(d < (2+4*lane_no+2) && d > (2+4*lane_no-2)){
					...
					// Check S value greater than our car and the gap(30 meters)
			...
			// Gardually decreasing / increasing the velocity
			if(too_close){
				ref_vel -= 0.224;
			...
			// Adding three points in Frenet coords spaced 30m
			vector<double> next_wp0 = getXY(car_s+30,(2+4*lane_no),map_waypoints_s,map_waypoints_x,map_waypoints_y);
			vector<double> next_wp1 = getXY(car_s+60,(2+4*lane_no),map_waypoints_s,map_waypoints_x,map_waypoints_y);
			vector<double> next_wp2 = getXY(car_s+90,(2+4*lane_no),map_waypoints_s,map_waypoints_x,map_waypoints_y);
			...
			// Ptsx and ptsy will have 5 points (2 previous points and 3 waypoints)
			// Shifting all points to car reference coords
			...
			// Pushing all the previous points left
			for(int i=0;i<previous_path_x.size();i++){
				next_x_vals.push_back(previous_path_x[i]);
				next_y_vals.push_back(previous_path_y[i]);
			}
			// Spliting the spline of Anchor points to get desired velocity
